...
$.extend( true, DataTable.ext.classes, {
	...
	length: {
		select: "dt-length-custom"
	},
	processing: {
		container: "dt-processing"
	},
	paging: {
		notActive: 'dt-pagiation-notactive',
		button: 'dt-pagiation-button',
		first: 'dt-pagiation-first',
		last: 'dt-pagiation-last',
		enabled: 'dt-pagiation-enabled',
		notEnabled: 'text-gray-300',
		active: 'dt-pagiation-active',
	},
...
DataTable.ext.renderer.layout.tailwindcss = function ( settings, container, items ) {
	var row = $( '<div/>', {
			"class": items.full ?
				' datatable-custom items-full items-cols-1' :
				' datatable-custom items-full items-cols-2'
		} )
		.appendTo( container );
	...
	$.each( items, function (key, val) {
		...
		if (val.table) {
			klass = ' datatable-custom table-key default';
		}
		else if (key === 'start') {
			klass = ' datatable-custom table-key key-start';
		}
		else if (key === 'end') {
			klass = ' datatable-custom table-key key-end';
		}
		else {
			klass = 'datatable-custom table-key key-center';
		}
