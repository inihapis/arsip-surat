...
@section('title', 'Daftar User')
                ...
                <div class="py-4 grid gap-3 md:flex md:justify-between md:items-center border-b border-gray-100">
                    ...
                    <div>
                        <div class="inline-flex gap-x-2">
                            ...
                            <!-- Tombol Tambah -->
                            <x-button onclick="addUser()"><i class="ti ti-circle-plus"></i>Tambah User</x-button>
                        </div>
                    </div>
                </div>
                <!-- End Header -->
                ...
                <div id="modal" class="fixed inset-0 z-50 hidden bg-gray-800 bg-opacity-50 items-center justify-center">
                    <div class="bg-white overflow-hidden rounded-lg shadow-lg w-11/12 md:w-1/3">
                        ...
                        <form id="modal-form">
                            ...
                            <div class="mb-4">
                                <label for="input-name" class="block text-sm font-bold text-gray-700">Nama User</label>
                                <x-input id="input-name" class="block mt-1 w-full" type="text" placeholder="Nama User" required />
                            </div>
                            <div class="mb-4">
    ...
    function addUser() {
        $('.modal-title').text('Tambah User'); // Ubah judul modal
    ...
    $(document).ready(function() {
        ...
        var table = $('#dataTable').DataTable({
            ...
            columns: [
                ...
                { data: null, render: function(data, type, row) {
                    return '<button type="submit" class="btn-icon-action tippy-button" data-tippy-content="Edit Data"" onclick="editUser(' + row.id + ')">' +
                   '<i class="ti ti-edit"></i>' +
                   '</button>' +
                   '<button type="submit" class="btn-icon-actionDelete tippy-button" data-tippy-content="Hapus Data" onclick="confirmDeleteUser(' + row.id + ')">' +
            ...
            columnDefs: [
                ...
                {
                    targets: [1, 2], // Indeks kolom yang ingin ditambahkan kelas p-3
                    createdCell: function(td, cellData, rowData, row, col) {
        ...
        $(document).on('submit', '#modal-form', function(e) {
            ...
            $.ajax({
                url: id ? `/user/update/${id}` : '/user/store', // Gunakan rute yang sesuai
                method: id ? 'PUT' : 'POST', // Gunakan PUT untuk update, POST untuk create
                data: { 
                    name: name, 
